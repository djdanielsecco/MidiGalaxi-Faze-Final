...
function listFader() {
	...
	for (var i = 0; i < CreFaders.length; i++) {
		...
		CreFaders[i].addEventListener('input', function () {
			...
			var popo = findlist.dataset.ind;
			console.log(findlist.dataset.ind + '   ' + findlist.id);
			portOut[devicesInfPor[popo]].sendControlChange(Number(devicesInfcc[popo]),findlist.value,devicesInfcha[popo]);
